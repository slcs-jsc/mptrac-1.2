...
typedef struct {
  ...
  /*! Output data format (0=ASCII, 1=netCDF, 2=netCDF spilt). */
  int atm_oformat;

  /*! number of parcels in each netcdf file. */
  int atm_netcdf_output_np;
...
typedef struct {
  ...
  /*! Zonal wind standard deviation [m/s]. */
  double usig[EX][EY][EP];

  /*! Meridional wind standard deviation [m/s]. */
  double vsig[EX][EY][EP];

  /*! Vertical wind standard deviation [hPa/s]. */
  double wsig[EX][EY][EP];
