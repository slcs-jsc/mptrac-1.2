...
int main(
  ...
  if (argc < 11)
    ERRMSG
      ("Give parameters: <min lon> <max lon> <min lat> <max lat> <min z> <max z> <allow out count> <outfile> <atm1a> <atm1b> [<atm2a> <atm2b> ...]");

  strcpy(atmStartName, argv[8] );
  if (argv[9][strlen(argv[9]) - 1] == 'c') {
    ctl.atm_iformat = 1;
  }
  minlon=atof(argv[1]);
  maxlon=atof(argv[2]);
